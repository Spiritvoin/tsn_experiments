...
$('.searchAutocomplete').click(function() {
 $('.searchText').toggle('slow', function() {
	...
	$('.logout').live('click',function() { 
		  ...
		  loading(str,overlay);
		  setTimeout("unloading()",1500);
		  setTimeout( "window.location.href='"+ $(this).find('a').attr('href')+"'", 2000 );
	  });
 ...
 function Processgraph(){
	...
	$('div.logout').hover(function(){
		  ...
		  $(this).find('img').animate({ opacity: 0.4 }, 200, function(){
			    $(this).attr('src','/kiosk/themes/tsn_yii_admin/images/'+name+'.png').animate({ opacity: 1 }, 500);									 
		 });
	},function(){
	...
	$('.fileDialog').live('focus',function(){
        ...
        $('.fileDialogCustom').live('focus',function(){
		  var callback,input =$(this).attr('id');
                    callback = $(this);
		  var type=$(this).attr('title');
		  fileDialogCallbackCustom(callback,type,input);										
	})
	...
	if(mybrowser.indexOf('Safari')>0){}		
	  ...
	  function fileDialogCallback(callback,type,input){
			$('<div id="finder_'+callback+'"/>').elfinder({
				 url : '../../../../themes/tsn_yii_admin/components/elfinder/connectors/php/connector-'+type+'.php', editorCallback : function(url) { $('#'+input).val(url);
				},closeOnEditorCallback : true, dialog : { title : 'File manager',modal : true,width : 700 }
			})							   
	  }
          function fileDialogCallbackCustom(callback,type,input){
//              console.log(callback,type,input);
			$('<div id="finder_'+callback+'"/>').elfinder({
				 url : '../../../../themes/tsn_yii_admin/components/elfinder/connectors/php/connector-'+type+'.php', editorCallback : function(url) { callback.val(url); callback.parents('.block_callback').find('.block_callback_img').html('<img  src="'+url+'"  width="180" height="180" alt="" />');
				},closeOnEditorCallback : true, dialog : { title : 'File manager',modal : true,width : 700 }
			})							   
	  }
	  function fileDialog(callback,type){
			$('<div id="finder_'+callback+'"/>').elfinder({
				  url : '../../../../themes/tsn_yii_admin/components/elfinder/connectors/php/connector-'+type+'.php',dialog : { title : 'File manager',modal : true,width : 700 }
...
function style_forms(el) {
      ...
      if ($(this).parent().next('select').attr('name')  == 'countryID'  ) {
              if ($(this).parent().next('select').find('option').eq(num).val() == 223) {
                            $("span.Zones_zoneID").show();
        } else {
             $("span.Zones_zoneID").hide();
        }
           ...
           if ($(this).parent().next('select').attr('id')  == 'User_countryID'  ) {
              if ($(this).parent().next('select').find('option').eq(num).val() == 223) {
                            $("span.Zones_zoneID").show();
        } else {
             $("span.Zones_zoneID").hide();
