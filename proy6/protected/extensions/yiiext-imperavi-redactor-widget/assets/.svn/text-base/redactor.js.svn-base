...
(function($){
	...
	var Redactor = function(element, options)
	{
		...
		this.opts = $.extend({
			...
			buttonsCustom: {'filemanager':{'title':'filemanager','func':'fileDialogCallback'}},
			buttonsAdd: [],
			buttons: ['html', '|', 'formatting', '|', 'bold', 'italic', 'deleted', '|', 'unorderedlist', 'orderedlist', 'outdent', 'indent', '|',
					'filemanager' /*'image'*/, /*'video'*/, 'file', 'table', 'link', '|',
...
//                            console.log(json);
		...
		scrolltop: function()
		{
			$('html,body').animate({ scrollTop: 0 }, 500);
                ...
                fileDialogCallback: function(callback){
                       var type = 'fileimport';

                       var obj_this=this;
                       $('<div id="finder_'+callback+'"/>').elfinder({
                           url : '../../../../themes/tsn_yii_admin/components/elfinder/connectors/php/connector-'+type+'.php',
                           editorCallback : function(url) {
                               var data="{\"filelink\":\""+url+"\"}";  /* console.log(data,"{\"filelink\":\""+url+"\"}");*/
                               obj_this._imageSet(data); 
                           },
