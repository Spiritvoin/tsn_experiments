    ...
    public function accessRules()
    {
        return array(
            array('allow', // allow all users to perform 'index' and 'view' actions
                'actions' => array('index', 'view', 'contact', 'captcha', 'InfoPage'),
                'users' => array('*'),
            ),
            array('allow', // allow authenticated user to perform 'create' and 'update' actions
                'actions' => array('create', 'update', 'AddLinkMenu'),
...
//            $vowels = array(" ", ',');
////            $permalink = urlencode(strtolower(str_replace($vowels, "-", $_POST['Page']['title'])));
...
//                        $model->permalink = urlencode($permalink) . '-1';
    ...
    public function actionContact()
    {
        ...
        if (isset($_POST['ContactForm'])) {
            ...
            if ($model->validate()) {
                $subject = "Contact from kiosk";
                $headers = "From: info@kiosk.trustingdomains.com";
                $body = $model->body . ' Send: ' . $model->email;
                mail('vladbbk@gmail.com', $subject, $body, $headers);
                Yii::app()->user->setFlash('contact', 'Thank you for contacting us. We will respond to you as soon as possible.');
                $this->refresh();
            }
        }
        $this->render('view_contact', array('model' => $model));
    }
